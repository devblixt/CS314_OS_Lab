...
int main(int argc, char *argv[])
{
    ...
    if (argc != 3)
    {
        cout << "Usage: " << argv[0] << " file time_slice" << endl;
        exit(1);
    }
    Cpu cpu;
    ...
    while (!(done == todo))
    {
        ...
        //SRTF interrupts everything
        if(system_time % time_slice == 0) {
            //cpu preemption check
            int time_left = cpu.completion_time - system_time;
            if(time_left > cpu_queue.top().value1){
                //preemption needed confirmed
                int process_id = cpu.process_id;
                PriorityElement preemptedProcess;
                preemptedProcess.value1 = time_left;
                preemptedProcess.value2 = process_id;
                cpu.process_id = cpu_queue.top().value2;
                cpu.completion_time = cpu_queue.top().value1 + system_time;
                cpu_queue.pop();
                cpu_queue.push(preemptedProcess);
            }
        }
        ++system_time;
    ...
    for(int i=0;i<size_p;i++){
        ...
        cout<<"Penalty ratio is : "<<penalty_ratio<<endl<<endl<<endl;
